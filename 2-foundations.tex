...
\]
It is a good exercise to check that these are actually valuations. 
\end{example}

Any field with valuation can be given a Hausdorff topology in a functorial way. 
This topology is not the naive definition via metrics, because we don't have an 
absolute value on the field. But formally, it's the same idea. We define a 
basis of open neighborhoods on the field $K$. For $\gamma\in \Gamma$, let 
\[
...
$U_\gamma(a)=\{a\}$, which is equivalent to the topology being discrete. Call 
two valuations \emph{equivalent} if they induce the same topology. It is known 
that two valuations are equivalent if and only if they induce ``independant'' 
valuation rings. Here, two valuation rings are dependent if\ldots. [There are 
easy characterizations for valuations that induce the same topology.] 

\begin{proposition}
Let $(K,\valuation)$ be a valued field. Give $K$ the topology induced by 
$\valuation$. Then 
\begin{enumerate}
\item
The field operations $(\cdot,+,(\cdot)^{-1})$ are continuous. 

\item
For all $x,y\in K$, $\gamma\in \Gamma$, if $x y\in U_{2\gamma}(0)$, then 
either $x\in U_\gamma(0)$ or $y\in U_\gamma(0)$. 
\end{enumerate}
\end{proposition}

Such topologies are called \emph{v-topologies}. There is an deep converse 
result: any v-topology comes from a valuation. [find source]
